...
Versão MPI+Pthreads sem input/output
********************************************************************/
#include "mpi.h"
#include <stdio.h>
#include <stdlib.h>
...
void write_to_file(){
    FILE * file;
    char * filename               = "output_mpi_pth.ppm"; /* Figura do output*/
    char * comment                = "# ";
...
void *mandelbrot_task(void *threadid){
    ...
    /* Cada thread ira fazer o loop em y com começo em i_y_0 e fim em i_y_end*/

    i_y_0 = i_y0 + tid*(i_yend-i_y0)/NUM_THREADS;       /* Inicio do loop em y */
    i_y_end = i_y0 + (tid+1)*(i_yend-i_y0)/NUM_THREADS; /* Fim do loop em y    */
    //printf("Thread %ld, %d %d, %d %d\n", threadid, i_y_0, i_y_end, i_y0, i_yend);    

    /*A thread ira fazer o loop com indices (i_y, i_x) em [i_y0, i_yend] x [0,i_x_max] */
    for(i_y = i_y_0; i_y < i_y_end; i_y++){
