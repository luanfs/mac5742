...
Versão MPI+OMP com input/output
********************************************************************/
#include "mpi.h"
#include <stdio.h>
#include <stdlib.h>
...
void write_to_file(){
    FILE * file;
    char * filename               = "output_mpi_omp.ppm"; /* Figura do output*/
    char * comment                = "# ";
...
i_y0 e i_yend (variaveis global nesse programa). 
O looping é feito usando OMP parallel for com total de threads dadas NUM_THREADS.
Esta é a tarefa de cada processo.
********************************************************************/
void mandelbrot(){
    double z_x;
    double z_y;
    ...
    shared (c_x_min, c_x_max, i_x_max, i_y_max, pixel_height, pixel_width, escape_radius_squared, iteration_max)\
    private (iteration, i_x, i_y, c_x, c_y, z_x, z_y, z_x_squared, z_y_squared)
